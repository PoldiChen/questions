...
#### 2. 微服务架构存在的问题及其解决方案？
调用链路复杂
部署、监控困难（docker、devops技术）
增加网络通信开销
集成测试困难
分布式问题（分布式缓存、分布式事务、分布式锁、分布式session）
进程间通讯和故障处理变得复杂（RPC框架，比如dubbo可以支持多种协议通信，springcloud可以支持restful调用）
服务数量多，测试、部署、监控变得困难（docker、devops技术、自动化运维工具）

...
#### 9. Java静态代理和动态代理？动态代理的实现方式？实际的应用？？？codee
应用：spring框架的AOP
...
#### 29. 如何创建一个不可变（immutable class）对象？JDK有哪些不可变对象？？？
(1) 类用final修饰<br>
...
#### 31. 使用递归的方法将一个字符串反转？code

#### 32. 不使用临时变量的情况下交换两个整数的值？code

#### 33. main方法不用static修饰会怎样？
...
#### 35. 反射的作用是什么？什么是动态语言？Java是不是动态语言？
...
#### 37. 注解的作用？自定义注解？（项目中的使用，参数检查）

#### 38. Socket服务端、客户端实例。code
...
#### 43. Float类型的变量定义float f = 2.3;是否正确？
不正确。精度不准确，应该加强制类型转换，float f = (float)2.3;或者float f = 2.3f;<br>
没加小数点默认是int，有加小数点默认是double。

#### 44. String编码UTF-8和GBK的区别？？？

#### 45. String编码UTF-8和GBK的区别？codee
...
#### 60. Java类可以定义析构函数（finalization）吗？？？

#### 61. finalize()方法什么时候被调用？
...
#### 63. Java 8，接口中可以有方法实现？？？

#### 64. 静态变量和实例变量的区别？
静态变量存储在方法区，实例变量存储在堆，其引用存储在当前线程栈中。

#### 65. String对象的intern()方法？code
先从常量池中查找是否存在该常量值，不存在则创建，已存在则直接返回。

#### 66. Java 7、8、9、10的新特性？？？
Java 7新特性：<br>

...
#### 4. 负载均衡实现算法？共享session共享？如何避免nginx单点故障？
...
#### 6. Nginx简介？Nginx的master和worker进程分别是什么？如何实现热部署？如何实现高并发？
...
#### 7. 短链接（网址）设计？？？question-java-web 10
...
#### 14. 分布式架构设计中的CAP理论？
数据一致性（Consistency）：所有节点在同一时间具有相同的数据<br>
可用性（Availability）：保证对于每个请求成功或失败都有响应<br>
分区容错性（Partition Tolerance）：系统中信息的丢失或失败不影响系统的运行<br>
